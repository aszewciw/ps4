    ...
    for(j = grid.ys; j < (grid.ys + grid.ym); j++) // Columns
    {
      ...
      for(i = grid.xs; i < (grid.xs + grid.xm); i++) // Rows
      {
        ...
        else
        {
          ...
          f *= ( 12.0 * x2 - 2.0 ) * (y4 - y2) * (z2 - z4) +
                ( 2.0 - 12.0 * y2 ) * (x2 - x4) * (z2 - z4) +
                ( 12.0 * z2 - 2.0 ) * (x2 - x4) * (y4 - y2);
...
compute_opt(KSP ksp, Mat A, Mat J, void * ctx)
{
  ...
  unsigned int k;
  for(k = grid.zs; k < (grid.zs + grid.zm); k++) // Columns
  {
    unsigned int j;
    for(j = grid.ys; j < (grid.ys + grid.ym); j++) // Columns
    {
      ...
      for(i = grid.xs; i < (grid.xs + grid.xm); i++)  // Rows
      {
        ...
        {
          v[0] = 2.f * (hxhydhz + hxhzdhy + hyhzdhx);
